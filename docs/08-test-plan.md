...
| `DOC-005` | 当前 Schema V6 SQL 可执行，且结构与 V1 → V6 迁移结果一致 |
...
macOS 0.3.0 先作为未公证测试构建：云端执行 Rust/Flutter 测试、Universal 二进制架构与签名校验、Release 程序启动、真实数据库初始化和 TCP 监听检查。Mac 输入框、平台初始化及 V5 → V6 迁移有自动测试。真实 Mac 与 Windows/Android/Linux 的局域网互传、Gatekeeper/本地网络授权、通知点击、剪贴板双向防循环和登录启动仍必须单独验收，未完成前不称为正式稳定版。
