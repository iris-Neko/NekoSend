...
0.4.1 发布四平台文件/图片粘贴与会话附件草稿：复制后在聊天输入框粘贴，可附带文字提交，见 [输入侧说明](docs/09-composer.md)。接收端交互与既有传输故障不在本次改动范围内。

0.3.2 修复 Windows 任务栏图标及遗留快捷方式冲突；调试版与正式版使用独立的任务栏标识。安装和窗口图标检查已接入自动发布，详见 [本版说明](packaging/release-0.3.2.md)。
