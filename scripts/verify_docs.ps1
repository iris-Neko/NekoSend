    ...
    if ($null -eq $sqlite) {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                foreach ($migrationName in @('migration_v1.sql', 'migration_v2.sql', 'migration_v3.sql', 'migration_v4.sql', 'migration_v5.sql', 'migration_v6.sql')) {
                ...
                if ($migrationSucceeded) {
                    ...
                    if (@(Compare-Object $documentedSchema $migratedSchema).Count -ne 0) {
                        Add-Failure 'The documented current DDL differs from the schema produced by V1 through V6 migrations.'
                    }
                }
            }
        } finally {
